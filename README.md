...
```
npx playwright@1.14.1 install && npx playwright@1.14.1 run-server
```
...
WORKDIR /root
RUN npm install playwright@1.14.1 && ./node_modules/.bin/playwright install
...
```
npx playwright@1.14.1 install && npx playwright@1.14.1 launch-server firefox
```
...
RUN apk add --no-cache --virtual .install-deps build-base curl git \
    && pip install git+https://github.com/microsoft/playwright-python@v1.14.1 \
    && pip install git+https://github.com/YusukeIwaki/playwright-python-remote \
